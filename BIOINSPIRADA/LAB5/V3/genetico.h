...
void genetico::reproducir(Individuo* root){
	...
	if(aptitud<=ALPHA and aptitud>0 and hijos.size()<cantidad and isnan(aptitud)==false){
		hijos.push_back(make_tuple(tmp,aptitud));
	}
}

bool genetico::mutar(Individuo* &tmp){
	Num random_muta;
	random_muta=(Num)(rand()%100)/100.0;
	if(random_muta<=0.3){
		if(tmp->tipo=="Operador"){
			cout<<"cambiando operador"<<endl;
			int random_pos=(int)(rand())/((int)RAND_MAX/operadores.length());
			cout<<tmp->operador<<endl;
			cout<<operadores[random_pos]<<endl;	
			...
			cout<<"cambiando Constante"<<endl;
			Num random=(Num)(rand()%100)/100.0;
			if(random<=0.5){
				...
				cout<<"cambiando Variable de constante"<<endl;
				tmp->tipo="Variable";
			}
			return true;
		}else if (tmp->tipo=="Variable" ){
			Num random=(Num)(rand()%100)/100.0;
			if(random<=0.5){
				tmp->tipo="Variable";
				cout<<"cambiando variable de variable"<<endl;
			}else{
				cout<<"cambiando constante de variable"<<endl;
				tmp->tipo="Constante";
				tmp->valor=(terminales)(rand())/((terminales)RAND_MAX/limite);
				Num random_signo=(Num)(rand()%100)/100.0;
				if(random_signo<0.5){
		...
		if(tmp->hijo[0] and mutar(tmp->hijo[0])==false){
			return mutar(tmp->hijo[1]);
		}else{
			return false	;
		}
	}
...
void genetico::cruzamiento(int pos1,int pos2){
	...
	if(aptitud1<=ALPHA and aptitud1>0 and hijos.size()<cantidad and isnan(aptitud1)==false){
		hijos.push_back(make_tuple(nodo1,aptitud1));
	}	
	if(aptitud2<=ALPHA and aptitud2>0 and hijos.size()<cantidad and isnan(aptitud2)==false){
...
void genetico::run(){
	...
	do{
		...
		while(hijos.size()<cantidad){
			...
			//cout<<"HOla"<<endl;
			cout<<endl<<"Individuo seleccionado"<<endl;
			mostrar_individuo(get<0>(individuos[pos1]));
			Num random_type=(Num)(rand()%100)/100.0;
			if(random_type>=0 and random_type<0.1){
				cout<<"reproduccion"<<endl;
				reproducir(get<0>(individuos[pos1]));
			}else if(random_type>=0.1 and random_type<0.7){
				...
				Individuo* tmp=new Individuo(get<0>(individuos[pos1]));
				
				if(mutar(tmp)==true){
					cout<<"El individuo MUTO"<<endl;
					cout<<endl<<"Individuo mutado"<<endl;
					//mostrar_individuo(tmp);
		...
		for(int j=0;j<this->cantidad;j++){
			if(get<1>(individuos[j])>get<1>(hijos[j]) or isnan(get<1>(individuos[j])))
				individuos[j]=hijos[j];
		}
		hijos.clear();
		//ALPHA-=0.0005;
		i++;
	}while(get<1>(individuos[0])>0.0002);
...
genetico::genetico(int cantidad,vector<Num> &entradas,vector<Num>&salidas,string operadores){
	...
	while (individuos.size()<cantidad)
	{
		Individuo *tmp=crear_arbol();
		Num aptitud=generar_aptitud(tmp);
		if(isnan(aptitud)==false)
