...
void genetico::busqueda_local(Individuo &a){
	Individuo Nuevo;
	int H=0;
	Nuevo =mutacion(a);
	while(H<=5 and Nuevo.aptitud>a.aptitud){
		cout<<endl<<"arbol"<<endl;
		H++;
		Nuevo =mutacion(a);
		mostrar_individuo(Nuevo);
	}
	if(Nuevo.aptitud<a.aptitud){
...
void genetico::reproducir(){
	...
	for (unsigned i = 0; i < poblacion/2; ++i)
	{
		...
		cout<<endl<<"Cruzamiento ";
			if(prob_cross>=random_cross	){
				...
				cout<<endl<<"Busqueda Local hijo 1"<<endl;
				busqueda_local(get<0>(hijos1));
				//mostrar_individuo(get<0>(hijos1));
				get<1>(hijos1).aptitud=generar_aptitud(get<1>(hijos1).datos);
				//mostrar_individuo(get<1>(hijos1));
				cout<<endl<<"Busqueda Local hijo 2"<<endl;
				busqueda_local(get<1>(hijos1));
				//mostrar_individuo(get<1>(hijos1));
	...
	cout<<endl<<"Seleccionando Nueva poblacion"<<endl;
	mostrar_poblacion();
	mostrar_hijos();
...
Individuo genetico::mutacion(Individuo a){
	int r1 = (int) (rand()) / (int) (RAND_MAX/a.datos.size());
	int r2 = (int) (rand()) / (int) (RAND_MAX/a.datos.size());
	while(r1==r2){
		r2 = (int) (rand()) / (int) (RAND_MAX/a.datos.size());
	}
	swap(a.datos[r2],a.datos[r1]);
	a.aptitud=generar_aptitud(a.datos);
	return a;
}
...
genetico::genetico(vector<vector<int>> & matriz,int poblacion){
	...
	std::sort (tmp.begin(), tmp.end(),Comp1());
	for(int i=0; i<poblacion;i++){
		individuos.push_back(tmp[i]);
	}
}
